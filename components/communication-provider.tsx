...
export default function CommunicationProvider() {
	...
	const [provider, setProvider] = useState<'slack'>('slack');
	const queryClient = useQueryClient();

	const { data, isLoading } = useQuery({
		queryKey: ['integrations', 'communication_provider'],
		queryFn: () =>
			fetch('/api/integrations?type=communication_provider').then((res) =>
				res.json(),
			),
	});

	const createMutation = useMutation({
		mutationFn: async (payload: { provider: 'slack'; token: string }) => {
	...
	return (
		<div className="space-y-6">
			...
			{emptyState && (
				<EmptyState
					title={t('No communication provider yet')}
					description={t(
						'Add a Slack integration to connect your communication channel.',
					)}
					actionLabel={t('Add Slack integration')}
					onAction={openCreate}
					...
					{data.map((integration: Integration) => (
						<div key={integration.id} className="flex flex-col gap-4">
							...
							{integration.status === 'connected' && (
								<CommunicationApiCredentials integration={integration} />
							)}
						</div>
					))}
				</div>
			...
			>
				<SheetContent className="w-full overflow-y-auto sm:max-w-2xl">
					<SheetHeader>
						<SheetTitle>
							{mode === 'create'
								? t('Add Slack integration')
								: t('Edit Slack integration')}
						</SheetTitle>
					</SheetHeader>
					<div className="space-y-4 p-4 pt-0">
						<div className="space-y-2">
							...
							<Select
								value={provider}
								onValueChange={(value) => setProvider(value as 'slack')}
								disabled={mode === 'edit'}
							>
						...
						<div className="space-y-2">
							...
							<InputPassword
								placeholder={t('Paste your Slack bot or user OAuth token')}
								value={token}
...
function CommunicationIntegrationCard({
	integration,
	onClick,
}: {
	integration: Integration;
						...
						<div className="flex items-center gap-3">
							...
							<div>
								<CardTitle className="text-sm">
									{integration.provider === 'slack' && t('Slack workspace')}
								</CardTitle>
...
function CommunicationApiCredentials({
	integration,
}: {
	integration: Integration;
}) {
		...
		<div className="mt-6 w-full">
			<Card>
				<CardHeader className="pb-3">
					...
					<CardDescription className="text-xs">
						{t('Update your Slack OAuth token if needed')}
					</CardDescription>
				</CardHeader>
				<CardContent className="space-y-4">
					<div className="space-y-2">
						<label className="text-sm font-medium">
							{t('Bot or user OAuth token')}
						</label>
						<InputPassword
